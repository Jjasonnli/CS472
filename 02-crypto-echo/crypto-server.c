...
static int service_client_loop(int client_sock) {
    ...
    for (;;) {
        ...
        pdu.direction = hdr[1];
        uint16_t nlen;
        memcpy(&nlen, &hdr[2], sizeof(uint16_t));
        pdu.payload_len = ntohs(nlen);

        if (pdu.payload_len > BUFFER_SIZE - sizeof(crypto_pdu_t)) {
        ...
        while (have < need) {
            ssize_t n = recv(client_sock, payload_dst + have, need - have, 0);
            if (n == 0) return RC_CLIENT_EXITED;
        ...
        while (psent < plen) {
            ...
            if (m < 0) { if (errno == EINTR) continue; perror("send payload"); return RC_CLIENT_EXITED; }
            psent += (size_t)m;
        }
    }
}
