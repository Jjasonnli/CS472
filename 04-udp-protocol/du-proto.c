...
int dprecv(dp_connp dp, void *buff, int buff_sz){
    ...
    while (totalRcv < buff_sz) {

        // Read a single PDU (header + payload) into the internal buffer.
        int rcvLen = dprecvdgram(dp, _dpBuffer, sizeof(_dpBuffer));

        if (rcvLen == DP_CONNECTION_CLOSED) {
            // If we have not received anything yet, propagate the close.
            // If we already have data, just return what we got.
            return (totalRcv == 0) ? DP_CONNECTION_CLOSED : totalRcv;
        }

        if (rcvLen < 0) {
            // Some protocol or I/O error; if we already have data, return that,
            // otherwise return the error code.
            return (totalRcv == 0) ? rcvLen : totalRcv;
        }

        dp_pdu *inPdu = (dp_pdu *)_dpBuffer;

        // Control PDUs (e.g., CLOSE, CONNECT/ACK) have dgram_sz == 0.
        // For a data recv we ignore them and keep waiting for data
        if (inPdu->dgram_sz == 0) {
            continue;
        }

        // Check that payload fits in caller's buffer.
        if (totalRcv + inPdu->dgram_sz > buff_sz) {
            return (totalRcv == 0) ? DP_BUFF_UNDERSIZED : totalRcv;
        }

        // Copy payload from internal buffer (after header) into caller's buffer.
        memcpy((char *)buff + totalRcv,
               _dpBuffer + sizeof(dp_pdu),
               inPdu->dgram_sz);

        totalRcv += inPdu->dgram_sz;

        if (inPdu->dgram_sz < dpmaxdgram() || totalRcv >= buff_sz) {
            break;
        }
    }
...
int dpsend(dp_connp dp, void *sbuff, int sbuff_sz) {
    ...
    // Allow arbitrary-length payload by sending multiple datagrams.
    // Each datagram carries at most dpmaxdgram() bytes of payload.
    ...
    while (remaining > 0) {
        int chunkSz = remaining;
        if (chunkSz > dpmaxdgram()) {
            chunkSz = dpmaxdgram();
        }

        int rc = dpsenddgram(dp, ptr, chunkSz);
        if (rc < 0) {
            return (totalSent == 0) ? rc : totalSent;
        }

        // dpsenddgram() returns the payload bytes sent
        if (rc != chunkSz) {
            // Short send is unexpected for UDP; warn and stop.
            fprintf(stderr,
                    "dpsend: short send: expected %d payload bytes, actually sent %d\n",
                    chunkSz, rc);
            totalSent += rc;
            break;
        }
